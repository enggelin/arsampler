...
Package `arsampler` provides a hassle-free way to implement the acceptance-rejection sampling. Given the nature of this package, however, the user needs to have a certain knowledge about statistics, especially about Monte Carlo integration and random number properties. Therefore, the intended main users for this package are researchers or students who want to experiment and test several distributions they have in hand and see how they match, without the need to code everything from scratch. All the user really needs to know beforehand are the pdf form of the target distribution and the parameters of their proposal distribution, and the package will take care of the rest. There are several limitations of this package in its current version (Version 1.0.0), however, which will be explained in the next section.


## Limitations

Considering the short time commitment to build this package, it still has some limitations and shortcomings. Some of the limitations of this package in the current stage are:
...
The usage of the functions in this package is relatively straightforward. There are 5 functions currently available in this version: `ar()` (the main function), `ar_print()`, `ar_summary()`, `ar_plot()`, and `ar_conv()`. The explanation for each functions are described below.
  ...
  * `ar_conv()` accepts one mandatory argument, which is an object of class `ar`. This function prepares the object `ar` for diagnostic plots. The output of this function is an object of class `ar_conv()`.
  
  * `ar_print()` allows the user to print the raw values from object of class `ar` or `ar_conv`. User then can view the values obtained from running the sampler.
  
  * `ar_summary()` accepts one mandatory argument, which is an object of class `ar` or `ar_conv`, the output from the function `ar()` and `ar_conv()`, respectively. If the object is of class `ar`, it returns an summary of the sampler, which will be explained in the next section **Interpreting the output**. If the object is of class `ar_conv`, it also returns a similar summary of the sampler, with a few differences. The functionality of this command can also be implemented using the command `summary()`, which, using its S3 object-oriented method, enables it to be modified based on the object we pass through it. Both of these functions return the same summaries of the object `ar` or `ar_conv`.
  
  * `ar_plot()` accepts one mandatory argument, which is an object of the class `ar` or `ar_conv` as the previous function. If the object is of class `ar`, this command returns an overlay for both the target and proposal densities plots in a single graph; whereas for an object of class `ar_conv`, it returns four diagnostic plots: convergence of acceptance rate, scatterplot of accepted y values, convergence of expected values, and convergence of variance.
...
The output above can also be achieved by using the commands `print()` and `summary()` respectively, which is an S3 object-oriented method in R. Those commands are overridden if the user passes an object of class `ar`, which is the object class of the output from the function `ar()`.
...
For convenience purpose, the examples above was only run for 10 iterations, therefore the output has to be interpreted in care, because a short iteration will not be enough to produce a valid output.
